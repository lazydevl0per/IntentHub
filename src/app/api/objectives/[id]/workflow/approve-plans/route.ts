...
export async function POST(
  ...
  if (workflow.status !== "AWAITING_PLAN_APPROVAL") {
    return badRequest("Workflow is not awaiting plan approval");
  }

  try {
    await continueAfterPlanApproval(workflow.id);
    const response = await getWorkflowResponse(id);
    return NextResponse.json(response);
  } catch (error) {
    return serverError("Failed to continue workflow", error);
  }
}
